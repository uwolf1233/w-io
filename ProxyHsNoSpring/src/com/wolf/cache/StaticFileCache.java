...
public class StaticFileCache {
	...
	public static ByteBuf getFile(String filePath){
		...
		try {
			...
			//return staticFileMap.get(filePath).copy();
			...
			for(File file : files){
				if(file.isDirectory()){
					...
					if(bytes!=null && bytes.length>0){
						...
						//abPath = abPath.replace(fPath, "");//替换掉前面的一截
						//abPath = abPath.substring(abPath.indexOf("fPath"), endIndex)
...
//						staticFileMap.put(abPath, bytebuf);//将数据放入堆外内存，提高传输效率
						//staticFileMap.put(abPath, PooledByteBufAllocator.DEFAULT.directBuffer().writeBytes(bytes));//将数据放入堆外内存，提高传输效率
						staticFileMap.put(abPath, Unpooled.directBuffer().writeBytes(bytes));
					}
				}
			}
		}
