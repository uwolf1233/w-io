...
import com.wolf.outin.javabean.IdMoneyToMoney;
...
public class MyJdbc {
	...
	public <T> List<T> queryForListObj(String sql,Object[] params,Class clazz,ReqResBean rrb){
		if(sql == null || sql.equals("")){
			throw new RuntimeException("sql is empty");
		}
		if(clazz == null){
	...
	public int javaBeanSaveEx(Object javabean,String tableName,ReqResBean rrb){
		...
		try {
			...
			for(int i=0;i<fieldsLen;i++){
				...
				Object osdata = fields[i].get(javabean);
				if(osdata == null){
					continue;
				}
				...
				wh += j==0 ? "?" : ",?";
				fieldnames += j==0 ? fileName : (","+fileName);
				olist.add(osdata);
				j++;
			}
	...
	public <T> int javaBeanListSaveEx(List<T> javabeans,String tableName,ReqResBean rrb) throws Exception {
		...
		try {
			...
			for(Object javabean : javabeans){
				...
				setPs(ps, fieldlist.toArray(),true);
			}
	...
	public List<Map<String,Object>> queryForMapList(String sql,Object[] params,ReqResBean rrb){
		if(sql == null || sql.equals("")){
			throw new RuntimeException("sql is empty");
		}
		Connection con = null;
