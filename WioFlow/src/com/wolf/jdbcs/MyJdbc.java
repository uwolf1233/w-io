...
import com.wolf.flow.javabean.IdMoneyToMoney;
...
public class MyJdbc {
	...
	public <T> int javaBeanListSaveEx(List<T> javabeans,String tableName,SaveFilter saveFiler,ReqResBean rrb) throws Exception {
		Connection con = rrb == null ? null : rrb.getCon();
		PreparedStatement ps = null;
		try {
			if(con==null){//表示没有事务
			...
			for(Object javabean : javabeans){
				...
				for(int i=0;i<fieldsLen;i++){
					String fileName = fields[i].getName();
					if(saveFiler.saveNameFilter(fileName)){
						continue;
					}
